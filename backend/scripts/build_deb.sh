...
PACKAGE_NAME=smartmeter-datacollector-configurator
...
poetry run -- shiv -c "${PACKAGE_NAME}" --root /var/lib/smartmeter-datacollector/.configurator_cache --compile-pyc -o "${SHIV_PACKAGE}" "${PY_DIST_DIR}/smartmeter_datacollector_configurator-${PACKAGE_VERSION}-"*.whl
echo "..done"
...
dh_make -y --single --createorig --templates "${WORK_DIR}/debian-tmpl" --copyright bsd --packagename "${PACKAGE_NAME}_${PACKAGE_VERSION}"
